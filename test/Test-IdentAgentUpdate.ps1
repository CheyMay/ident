...
if ([string]::IsNullOrWhiteSpace($PackagePath)) {
    $PackagePath = Join-Path $PSScriptRoot '..\agent\ident-db-agent\dist\ident-desktop-2.4.0.zip'
}
...
try {
    ...
    $success = Invoke-UpdaterProcess -Archive $PackagePath -Version '2.4.0' -Hash $packageHash
    ...
    if ([string]$updatedConfig.agent.version -ne '2.4.0') { throw 'Agent version was not updated.' }
    ...
    if ([string]$rolledBackConfig.agent.version -ne '2.4.0') { throw 'Configuration rollback did not preserve the installed version.' }
